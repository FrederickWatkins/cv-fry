...
extern "C" {
    ...
    void valu_set_word(Valu* dut, uint8_t val) {dut->word = val; }
    void valu_set_funct3(Valu* dut, uint8_t val) { dut->funct3 = val; }
    void valu_set_funct7(Valu* dut, uint8_t val) { dut->funct7 = val; }

    // Port Setters (32-bit)
    void valu_set_operand_1(Valu* dut, uint64_t val) { dut->operand_1 = val; }
    void valu_set_operand_2(Valu* dut, uint64_t val) { dut->operand_2 = val; }

    // Port Getters
    uint64_t valu_get_result(Valu* dut) { return dut->result; }
